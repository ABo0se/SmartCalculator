// physics_engine.js
// Extracted from smart_calculator.js as part of the multithreading refactor.
// This file contains ONLY pure computation: the ball-flight physics engine
// (Vector3D/Ball/Club/Wind/QuadTree/find_power) plus the AIM-refinement
// solver (solveShot) and the numerical-derivative "Diff" helpers.
// It touches no DOM APIs, so it can be loaded identically:
//   - on the main thread, via <script src="physics_engine.js"></script>
//   - inside a Web Worker, via importScripts('physics_engine.js')
// Each execution context (main thread, or each individual worker) gets its
// own independent copy of the module-level singletons (ball/club/wind), so
// running this in multiple workers concurrently is safe: each worker only
// ever mutates its own copies.
...
// ---- solveShot: AIM-refinement fixed-point solver ----
...
// ---- Numerical-derivative "Diff" helpers ----
