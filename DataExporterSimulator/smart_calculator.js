...
class QuadTree {
    ...
    getValuesDegree(degree, option = 0) {
        ...
        if (option == 0) {
            obj.cos = 1.0; obj.rad = 0.0; obj.sin = 0.0; obj._C = 0.0;
            obj._10 = Math.cos(degree); obj._14 = Math.sin(degree) * -1;
            obj.neg_sin = 0.0; obj.neg_rad = Math.sin(degree); obj.cos2 = obj._10;
            obj._24 = 0.0; obj._28 = 0.0; obj._2C = 0.0;
        } else if (option == 1) {
            obj.cos = Math.cos(degree); obj.rad = 0.0; obj.sin = Math.sin(degree);
            obj._C = 0.0; obj._10 = 1.0; obj._14 = 0.0; obj.neg_sin = obj.sin * -1;
            obj.neg_rad = 0.0; obj.cos2 = obj.cos; obj._24 = 0.0; obj._28 = 0.0; obj._2C = 0.0;
        }
        return obj;
    }
    ...
    ballProcess(steptime, final = undefined) {
        ...
        if (this.shot == SHOT_TYPE.COBRA && this.cobra_init < 0) {
            if (this.percentShot_sqrt < Math.sqrt(0.8)) this.percentShot_sqrt = Math.sqrt(0.8);
            if (this.ball.count == 0) {
                this.ball.velocity.y = 0.0;
                this.ball.velocity.normalize().multiplyScalar(this.power_factor_shot);
            }
            const diff = this.ball.position.clone().sub(this.ball_position_init).length();
            const cobra_init_up = ((this.power_range_shot * this.percentShot_sqrt) - 100.0) * 3.2;
...
function simulate_shot(power_player, club_info, shot, power_shot, altura, vento, angulo, terreno, spin, curva, slope, percentShot) {
    const simulate = (distanceEstimate) => {
        ...
        vclub.type_distance = calculeTypeDistance(distanceEstimate);

        const slope_mira_rad = slope instanceof Vector3D ? slope.y : 0.0;
        ...
        if (count >= limit_checking || copy_ball.position.y <= altura_colision)
            return null;
        const heightDelta = vball.position.y - copy_ball.position.y;
        if (Math.abs(heightDelta) < 1e-6)
            return null;
        const last_step = Math.abs((altura_colision - copy_ball.position.y) / heightDelta);
        vball.copy(copy_ball);
        qt.ballProcess(_00D083A0, _00D083A0 * last_step);
        return { z: vball.position.z, x: vball.position.x };
    };

    const resultA = simulate(58.0);
    if (!resultA)
        return { distance: -1, desvio: 0, pb: 0 };
    const distanceYards = resultA.z / 3.2;
    if (calculeTypeDistance(distanceYards) === calculeTypeDistance(58.0)) {
        const desvioYards = resultA.x * DESVIO_SCALE_PANGYA_TO_YARD;
        return { distance: distanceYards, desvio: desvioYards, pb: desvioByDegree(desvioYards, distanceYards) / YARDS_TO_PB };
    }
    const resultB = simulate(distanceYards);
    if (!resultB)
        return { distance: distanceYards, desvio: resultA.x * DESVIO_SCALE_PANGYA_TO_YARD, pb: (resultA.x * DESVIO_SCALE_PANGYA_TO_YARD) / YARDS_TO_PB };
    const desvioYards = resultB.x * DESVIO_SCALE_PANGYA_TO_YARD;
    const finalDistanceYards = resultB.z / 3.2;
    return { distance: finalDistanceYards, desvio: desvioYards, pb: desvioByDegree(desvioYards, finalDistanceYards) / YARDS_TO_PB };
}
...
// Which of the 8 sweepable fields map to which simulation input key, plus the id prefix
// used by the two range grids ("start"/"end") in the page.
const SWEEP_FIELDS = {
    power_used: { key: 'power_used', label: 'Power', idPrefix: 'power_used', checkboxId: 'powerusedcheckbox', summaryLabel: 'Power' },
...
const SWEEP_TYPE_ORDER = ['power_used', 'height', 'wind', 'degree', 'slope', 'ground', 'spin', 'curve'];
...
function solveAim(input_values) {
    ...
    vclub.init(input_values.club_info);
    vclub.type_distance = calculeTypeDistance(input_values.power_used);
    const powerRange = vclub.getRange(input_values.power_player.options, input_values.power_player.pwr, input_values.power_shot);
    const percent = powerRange > 0 ? Math.max(0, Math.min(1, input_values.power_used / powerRange)) : 0;
    const simulation = simulate_shot(
        input_values.power_player, input_values.club_info, input_values.shot, input_values.power_shot,
        input_values.height, input_values.wind, input_values.degree, input_values.ground,
        input_values.spin, input_values.curva, input_values.slope, percent
    );
    if (simulation.distance < 0)
...
// Builds the small set of "display" values (Pow %, travel distance, AIM, HWI) from a
// solved shot the exact same way the interactive calculator's result panel does.
function toDisplayRow(input_values, solved, dis, aimX) {
    ...
    const powYard = r.distance;
    const powerUsedYard = input_values.power_used;
    const hwi = r.pb * dis;
...
function localDerivative(baseParams, dis, aimX, key, targetDistance) {
    const eps = 0.1;
    ...
    const powerP = getPowerUsedForDistance(plus, targetDistance);
...
function computeSweepRow(fixedParams, varKey, value, dis, aimX, baseline, includeSensitivity) {
    ...
    row.powerUsedYard = disp.powerUsedYard;
    row.distance = disp.distance;
    row.aim = disp.aim;
    row.hwi = disp.hwi;
    ...
    if (baseline) {
        row.deltaPow = disp.powerUsedYard - baseline.powerUsedYard;
...
function getCheckedTypesInOrder() {
    ...
    for (const type of SWEEP_TYPE_ORDER) {
        const el = document.getElementById(SWEEP_FIELDS[type].checkboxId || type + 'checkbox');
        if (el && el.checked)
            types.push(type);
    }
...
function getPowerUsedForDistance(params, distance) {
    const solved = find_power(
        params.power_player, params.club_info, params.shot, params.power_shot,
        distance, params.height, params.wind, params.degree, params.ground,
        params.spin, params.curva, params.slope
    );
    if (solved.power === -1)
        return null;
    return solved.power * solved.power_range;
}

function getPowerUsedForDistanceAtHeightZero(params, distance) {
    return getPowerUsedForDistance(Object.assign({}, params, { height: 0 }), distance);
}
...
async function applyHeightAdjustmentsToSweep(sweep, fixedParams, cache, dis, aimX) {
    ...
    if (!canUseSweepWorkers()) {
        for (const item of items) {
            const powerAtHeightZero = getPowerUsedForDistanceAtHeightZero(item.params, item.row.distance);
            if (powerAtHeightZero === null)
                continue;
            const height = item.params.height;
            const currentHwiNorm = hwiNorm(item.row.hwi, item.params.wind, item.params.degree);
            const baselineParams = Object.assign({}, item.params, { height: 0 });
            const baselineHwiNorm = hwiNorm(item.baseline.hwi, baselineParams.wind, baselineParams.degree);
            item.row.hwiNorm = currentHwiNorm;
            item.row.deltaPow = item.row.powerUsedYard - powerAtHeightZero;
            item.row.deltaHwi = item.row.hwi - item.baseline.hwi;
            item.row.h = height !== 0 ? item.row.deltaPow / height : null;
            item.row.hwiAdj = height !== 0 ? (currentHwiNorm - baselineHwiNorm) / height : null;
        }
        return;
    }
...
async function exportSweep() {
    ...
    shared.clubConf = computeClubConfLabel(shared, 58.0);
    ...
    const appendSweepTables = (sweep, isRoundTrip) => {
        ...
        for (const block of sweep.blocks) {
            if (block.label !== null) {
                ...
                aoa.push(['', blockLabel]);
            } else if (sweepVar1.type !== 'power_used') {
                aoa.push(['', `Power : ${fixedFieldValues.power_used}`]);
            }
        ...
        for (let row = headerRow + 1; row <= aoa.length; row++) {
            ...
            for (let column = 1; column <= headerValues.length; column++) {
                ...
                const format = column === 1
                    ...
                    : header === 'Pow (%)' || header === 'Distance (y)' || header === 'ΔPow'
                    ? '0.000'
                    : '0.0000';
