...
function simulate_shot(power_player, club_info, shot, power_shot, altura, vento, angulo, terreno, spin, curva, slope, percentShot) {
    ...
    const simulate = (distanceEstimate) => {
        const altura_colision_raw = altura * 1.094 * 3.2;
        ...
        const altura_colision = altura_colision_raw + ball_radius;

        if (club != undefined)
            vclub.init(club_info);

        const slope_mira_rad = slope instanceof Vector3D
            ? slope.y
            : 0.0;
        ...
        const limit_checking = 100000; // safety cap so a shot that never crosses altura_colision can't loop forever
        ...
        // The interpolation below only makes sense if the loop actually bracketed a real
        // crossing: copy_ball above altura_colision, vball below it. If the shot never had
        // enough power/height to climb above altura_colision in the first place, the loop
        // instead exits once num_max_height resolves while the ball is already resting near
        // the ground (both samples far below altura_colision, close to each other). Treat
        // that as "this shot can't reach the target's elevation" instead of interpolating
        // through it.
        ...
        // Near-zero denominator means the two sampled points are essentially the same
        // height (e.g. the ball is rolling/settling rather than actively descending through
        // altura_colision) -- dividing here would blow last_step up to a huge, meaningless
        // multiplier and catapult the ball to a nonsense landing position.
    ...
    const firstEstimate = 58.0;
    const resultA = simulate(firstEstimate);
    if (!resultA)
        return { distance: -1, desvio: 0, pb: 0 };

    const distanceYards = resultA.z / 3.2;
    const typeDistance = calculeTypeDistance(distanceYards);

    if (typeDistance === calculeTypeDistance(firstEstimate)) {
        const desvioYards = resultA.x * DESVIO_SCALE_PANGYA_TO_YARD;
        return {
            distance: distanceYards,
            desvio: desvioYards,
            pb: desvioByDegree(desvioYards, distanceYards) / YARDS_TO_PB
        };
    }
    ...
    return {
        distance: finalDistanceYards,
        desvio: desvioYards,
        pb: desvioByDegree(desvioYards, finalDistanceYards) / YARDS_TO_PB
    };
}
...
function calc(el) {
    ...
    let power_used = checkValidInput(document.getElementById('power_used').value);
    ...
    // Convert input in yards into a normalized power percent using the selected club range
    const tempClub = new Club();
    tempClub.init(club);
    tempClub.type_distance = calculeTypeDistance(power_used);
    const club_range = tempClub.getRange({
        auxpart: auxpart_pwr,
        mascot: mascot_pwr,
        card: card_pwr,
        ps_auxpart: 0,
